    ...
    if liked_message in g.user.likes:
        ...
        for like in g.user.likes:
            if like != liked_message:
                liked_message_array.append(like)
        g.user.likes = liked_message_array
    else:
...
def toggle_message(message_id):
    """toggle message from user"""
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")

    message = Message.query_or_404(message_id)
    if message.user_id == g.user.id:
        return abort(403)
    if message in g.user.likes:
        message_array = [like for like in g.user.likes if like != message]
        g.user.likes = message_array
    else:
